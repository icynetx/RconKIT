...
def scan_nmap(target: str, timeout: int, report: ReconReport, ports: str | None, deep: bool) -> None:
    ...
    if deep:
        ...
        args.extend([*NMAP_TIMING_ARGS, target])
        ...
        fallback_args = ["nmap", "-n", "-oN", "-", "-Pn", "-T3", "--max-retries", "3", "--host-timeout", f"{fallback_timeout}s", "--open", "-p", fallback_ports, target]
        ...
        if open_ports:
            service_args = ["nmap", "-n", "-oN", "-", "-sV", "--version-light", "-Pn", "-T3", "--max-retries", "2", "--host-timeout", f"{fallback_timeout}s", "-p", ",".join(open_ports), target]
    ...
    discovery_args = ["nmap", "-n", "-oN", "-", "-Pn", "-T4", "--max-retries", "1", "--host-timeout", f"{discovery_timeout}s", "--open", "-p", selected_ports, target]
    ...
    if not open_ports and not ports and report.profile == "fast":
        ...
        fallback_args = ["nmap", "-n", "-oN", "-", "-Pn", "-T3", "--max-retries", "3", "--host-timeout", f"{fallback_timeout}s", "--open", "-p", WEB_FALLBACK_PORTS, target]
        fallback = run_cmd(fallback_args, effective_timeout)
        report.commands.append(command_record(fallback))
    ...
    service_args = ["nmap", "-n", "-oN", "-", "-sV", "--version-light", "-Pn", "-T4", "--max-retries", "1", "--host-timeout", f"{discovery_timeout}s", "-p", ",".join(open_ports), target]
    service = run_cmd(service_args, effective_timeout)
    report.commands.append(command_record(service))
...
def scan_web_stack(report: ReconReport, timeout: int, aggressive: bool) -> None:
    ...
    if httpx_bin:
        result = run_cmd([httpx_bin, "-silent", "-title", "-tech-detect", "-status-code", "-follow-redirects", "-u", urls[0]], timeout)
...
def scan_extra_modules(report: ReconReport, timeout: int, modules: set[str], aggressive: bool, raw_dir: Path | None = None) -> None:
    ...
    if not target_has_scan_endpoint(report):
        ...
        return
    if "web" in modules:
        scan_web_stack(report, timeout, aggressive)
    if "http-detail" in modules:
        scan_http_detail(report, timeout, raw_dir)
    if "tls" in modules:
        scan_tls(report, timeout, aggressive)
    if "screenshots" in modules:
        scan_screenshots(report, timeout, raw_dir)
    if "templates" in modules:
        scan_templates(report, timeout, raw_dir)
...
def scan_http_detail(report: ReconReport, timeout: int, raw_dir: Path | None = None) -> None:
    urls = web_urls(report)
    if not urls:
    ...
    for index, url in enumerate(urls[:4], 1):
        if which_tool("curl"):
            result = run_cmd(["curl", "-k", "-L", "-I", "--max-time", str(min(timeout, 20)), url], min(timeout, 25))
    ...
    if which_tool("katana"):
        result = run_cmd(["katana", "-silent", "-no-color", "-depth", "1", "-u", urls[0]], min(timeout, 45))
...
def scan_screenshots(report: ReconReport, timeout: int, raw_dir: Path | None = None) -> None:
    urls = web_urls(report)
    if not urls:
        return
...
def scan_templates(report: ReconReport, timeout: int, raw_dir: Path | None = None) -> None:
    urls = web_urls(report)
    if not urls:
        return
    if not which_tool("nuclei"):
        ...
        return
    result = run_cmd(["nuclei", "-silent", "-no-color", "-severity", "info,low,medium,high,critical", "-u", urls[0]], min(timeout, 90))
