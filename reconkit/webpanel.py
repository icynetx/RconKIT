...
<section id="ai" class="panel"><div class="grid"><div class="card"><h2>AI Settings</h2><form onsubmit="saveAI(event)"><label>Endpoint URL</label><input id="ai_endpoint"><label>Model</label><input id="ai_model"><label>API Key Env</label><input id="ai_env"><label>Direct API Key (local only)</label><input id="ai_key" placeholder="leave empty to keep/clear direct key"><div class="row"><div><label>Temperature</label><input id="ai_temp" type="number" step="0.01"></div><div><label>Max Tokens</label><input id="ai_tokens" type="number"></div></div><label>System Prompt</label><textarea id="ai_prompt"></textarea><button class="btn" type="submit">Save AI Config</button> <button type="button" class="btn secondary" onclick="testAI()">Test AI</button></form></div><div class="card"><h2>Current AI Config</h2><pre id="aiBox">Loading...</pre></div></div></section>
...
const $=id=>document.getElementById(id);function tab(id){document.querySelectorAll('.panel').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));$(id).classList.add('active');event?.target?.classList?.add('active');if(id==='history')loadHistory();if(id==='presets')loadPresets();if(id==='deps')loadDeps();if(id==='ai')loadAI();}
...
function renderScan(d){let r=d.report||{};let ports=(r.nmap_ports||[]).filter(p=>p.state==='open');let h=`<div class="actions"><a class="btn secondary" href="/api/scan/${d.scan_id}/report/json" target="_blank">JSON</a><a class="btn secondary" href="/api/scan/${d.scan_id}/report/markdown" target="_blank">Markdown</a><a class="btn secondary" href="/api/scan/${d.scan_id}/report/html" target="_blank">HTML</a>${d.status==='completed'?`<button class="btn secondary" onclick="runAI('${d.scan_id}')">Run AI</button>`:''}</div>`;h+=`<p>${statusBadge(d.status)} Target: <b>${d.target}</b> • Mode: ${d.mode} • Preset: ${d.preset} • Elapsed: ${d.elapsed||0}s</p>`;if(d.error)h+=`<pre>${d.error}</pre>`;h+=`<h3>Resolved IPs</h3><pre>${(r.resolved_ips||[]).join('\n')||'none'}</pre>`;h+=`<h3>Open Ports</h3>`;if(ports.length){h+='<table><tr><th>Port</th><th>Service</th><th>Version</th></tr>';ports.forEach(p=>h+=`<tr><td>${p.port}</td><td>${p.service}</td><td>${p.version||''}</td></tr>`);h+='</table>'}else h+='<p class="note">No open ports recorded.</p>';h+=`<h3>DNS</h3><pre>${JSON.stringify(r.dns_records||{},null,2)}</pre>`;h+=`<h3>Extra Tools</h3><pre>${JSON.stringify((r.extras||[]).map(e=>({module:e.module,tool:e.tool,ok:e.ok,missing:e.missing,summary:e.summary})),null,2)}</pre>`;h+=`<h3>Notes</h3><pre>${(r.notes||[]).join('\n')||'none'}</pre>`;if(d.ai_output)h+=`<h3>AI Analysis</h3><pre>${d.ai_output}</pre>`;return h;}
...
async function savePreset(e){e.preventDefault();let args={};try{args=$('pargs').value.trim()?JSON.parse($('pargs').value):{}}catch(err){alert('Tool args must be valid JSON');return}let body={name:$('pname').value,base:$('pbase').value,strategy:$('pstrategy').value,description:$('pdesc').value,tool_args:args};try{await api('/api/presets',{method:'POST',body:JSON.stringify(body)});loadPresets();alert('Preset saved')}catch(err){alert(err.message)}}async function deletePreset(n){if(confirm('Delete preset '+n+'?')){await api('/api/presets/'+encodeURIComponent(n),{method:'DELETE'});loadPresets()}}
...
async function loadAI(){let d=await api('/api/ai/config');$('aiBox').textContent=JSON.stringify(d.config,null,2);$('ai_endpoint').value=d.config.endpoint_url||'';$('ai_model').value=d.config.model||'';$('ai_env').value=d.config.api_key_env||'';$('ai_temp').value=d.config.temperature||0.2;$('ai_tokens').value=d.config.max_tokens||3200;$('ai_prompt').value=d.config.system_prompt||'';$('ai_key').value=''}
async function saveAI(e){e.preventDefault();let cfg={endpoint_url:$('ai_endpoint').value,model:$('ai_model').value,api_key_env:$('ai_env').value,temperature:$('ai_temp').value,max_tokens:$('ai_tokens').value,system_prompt:$('ai_prompt').value};if($('ai_key').value)cfg.api_key=$('ai_key').value;try{await api('/api/ai/config',{method:'POST',body:JSON.stringify(cfg)});loadAI();alert('AI config saved')}catch(err){alert(err.message)}}async function testAI(){try{let d=await api('/api/ai/test',{method:'POST',body:'{}'});alert('AI OK: '+d.reply)}catch(err){alert(err.message)}}
loadPresets();
</script></body></html>'''
...
def ensure_db() -> sqlite3.Connection:
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS scans (
            ...
            elapsed REAL, report_json TEXT, ai_output TEXT, error TEXT
        )
    """)
    conn.commit()
    return conn
...
def insert_scan(params: dict[str, Any]) -> str:
    ...
    conn().execute("""
        INSERT INTO scans (scan_id,target,mode,modules,preset,ports,timeout,aggressive,no_whois,ai,show_commands,raw_dir,status,started_at,elapsed)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,0)
    """, (scan_id, params["target"], params.get("mode", "fast"), params.get("modules", "safe"), params.get("preset", "standard"), params.get("ports"), int(params.get("timeout") or 90), int(bool(params.get("aggressive"))), int(bool(params.get("no_whois"))), int(bool(params.get("ai"))), int(bool(params.get("show_commands"))), params.get("raw_dir"), "queued", now))
    conn().commit()
    return scan_id
...
def list_scans() -> list[dict[str, Any]]:
    rows = conn().execute("SELECT scan_id,target,mode,preset,status,started_at,elapsed FROM scans ORDER BY started_at DESC").fetchall()
...
def run_scan_worker(scan_id: str) -> None:
    ...
    try:
        ...
        update_scan(scan_id, status="completed", elapsed=round(report.elapsed_seconds, 2), report_json=report_json)
        if scan.get("ai"):
            run_ai_for_scan(scan_id, report)
    except Exception as exc:
        update_scan(scan_id, status="failed", elapsed=round(time.monotonic() - started, 2), error=str(exc))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/api/status":
                return self.send_data({"app": APP, "version": VERSION, "author": AUTHOR, "website": WEBSITE})
            if path == "/api/scans":
