...
async function api(path,opts={}){opts.headers=Object.assign({'Content-Type':'application/json'},opts.headers||{});const r=await fetch(path,opts);const t=await r.text();let j=null;try{j=t?JSON.parse(t):null}catch(e){if(!r.ok)throw new Error(t||('HTTP '+r.status));throw new Error('Invalid JSON response from '+path)}if(!r.ok)throw new Error((j&&j.error)||t||('HTTP '+r.status));if(j===null)throw new Error('Empty response from '+path);return j;}
...
async function startScan(e){e.preventDefault();const body={target:$('target').value.trim(),mode:$('mode').value,preset:$('preset').value,modules:$('modules').value||'safe',ports:$('ports').value||null,timeout:Number($('timeout').value||90),raw_dir:$('rawdir').value||null,aggressive:$('aggressive').checked,no_whois:$('nowhois').checked,ai:$('ai').checked,show_commands:$('cmd').checked};try{const d=await api('/api/scan',{method:'POST',body:JSON.stringify(body)});if(!d||!d.scan_id)throw new Error('Scan did not return a scan_id');$('result').innerHTML=`${statusBadge('queued')} <span class="mono">${esc(d.scan_id)}</span>`;poll(d.scan_id);refreshStatus();}catch(err){$('result').innerHTML=`<div class="errorbox">${esc(err.message)}</div>`;alert(err.message)}}
function poll(id){let misses=0;let timer=setInterval(async()=>{try{let d=await api('/api/scan/'+encodeURIComponent(id));if(!d||typeof d!=='object')throw new Error('Invalid scan response');$('result').innerHTML=renderScan(d);misses=0;if(['completed','failed'].includes(d.status)){clearInterval(timer);refreshStatus()}}catch(e){misses++;$('result').innerHTML=`<div class="errorbox">Live update failed: ${esc(e.message)}<br>Scan ID: <span class="mono">${esc(id)}</span></div>`;if(misses>=3){clearInterval(timer);refreshStatus()}}},1800)}
function esc(v){return String(v??'').replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]))}
function renderScan(d){if(!d||typeof d!=='object')return '<div class="errorbox">Scan response was empty or invalid.</div>';let r=d.report||{};let ports=(r.nmap_ports||[]).filter(p=>p.state==='open');let extras=r.extras||[];let notes=r.notes||[];let h=`<div class="actions"><a class="btn secondary" href="/api/scan/${d.scan_id}/report/json" target="_blank">JSON</a><a class="btn secondary" href="/api/scan/${d.scan_id}/report/markdown" target="_blank">Markdown</a><a class="btn secondary" href="/api/scan/${d.scan_id}/report/html" target="_blank">HTML</a>${d.status==='completed'?`<button class="btn secondary" onclick="runAI('${d.scan_id}')">Run AI</button>`:''}</div>`;h+=`<p>${statusBadge(d.status)} <b>${esc(d.progress||'')}</b></p>`;if(d.status==='running'||d.status==='queued')h+=`<div class="progress"><i></i></div>`;h+=`<div class="metrics"><div class="metric"><span>Target</span><b>${esc(d.target)}</b></div><div class="metric"><span>Open Ports</span><b>${ports.length}</b></div><div class="metric"><span>Resolved IPs</span><b>${(r.resolved_ips||[]).length}</b></div><div class="metric"><span>Elapsed</span><b>${d.elapsed||0}s</b></div></div>`;h+=`<p class="note">Mode: ${esc(d.mode)} • Preset: ${esc(d.preset)} • Scan ID: <span class="mono">${esc(d.scan_id)}</span></p>`;if(d.error)h+=`<pre>${esc(d.error)}</pre>`;h+=`<div class="sectionTitle">Resolved IPs</div><pre>${esc((r.resolved_ips||[]).join('\n')||'none')}</pre>`;h+=`<div class="sectionTitle">Open Ports & Services</div>`;if(ports.length){h+='<table><tr><th>Port</th><th>State</th><th>Service</th><th>Version</th></tr>';ports.forEach(p=>h+=`<tr><td class="mono">${esc(p.port)}</td><td>${esc(p.state)}</td><td>${esc(p.service)}</td><td>${esc(p.version||'')}</td></tr>`);h+='</table>'}else h+='<p class="note">No open ports recorded yet.</p>';h+=`<div class="sectionTitle">DNS Records</div><pre>${esc(JSON.stringify(r.dns_records||{},null,2))}</pre>`;h+=`<div class="sectionTitle">Extra Tooling</div>`;if(extras.length){h+='<table><tr><th>Module</th><th>Tool</th><th>Status</th><th>Summary</th></tr>';extras.forEach(e=>h+=`<tr><td>${esc(e.module)}</td><td>${esc(e.tool)}</td><td>${statusBadge(e.missing?'missing':(e.ok?'ok':'warn'))}</td><td>${esc((e.summary||[]).join(' | '))}</td></tr>`);h+='</table>'}else h+='<p class="note">No extra tool output yet.</p>';h+=`<div class="sectionTitle">Notes</div><pre>${esc(notes.join('\n')||'none')}</pre>`;if(d.ai_output)h+=`<div class="sectionTitle">AI Analysis</div><pre>${esc(d.ai_output)}</pre>`;return h;}
async function loadHistory(){try{let d=await api('/api/scans');let scans=Array.isArray(d.scans)?d.scans:[];if(!scans.length){$('historyBox').innerHTML='<div class="empty">No scans yet. Run a scan first.</div>';return}let h='<table><tr><th>ID</th><th>Target</th><th>Mode</th><th>Preset</th><th>Status</th><th>Progress</th><th>Started</th><th>Actions</th></tr>';scans.forEach(s=>h+=`<tr><td class="mono">${esc(s.scan_id)}</td><td>${esc(s.target)}</td><td>${esc(s.mode)}</td><td>${esc(s.preset)}</td><td>${statusBadge(s.status)}</td><td>${esc(s.progress||'')}</td><td>${esc(s.started_at||'')}</td><td><button class="btn secondary" onclick="openScan('${esc(s.scan_id)}')">Open</button> <button class="btn danger" onclick="deleteScan('${esc(s.scan_id)}')">Delete</button></td></tr>`);$('historyBox').innerHTML=h+'</table>'}catch(e){$('historyBox').innerHTML=`<div class="errorbox">History failed: ${esc(e.message)}</div>`}}
async function openScan(id){tab('scan');try{let d=await api('/api/scan/'+encodeURIComponent(id));$('result').innerHTML=renderScan(d)}catch(e){$('result').innerHTML=`<div class="errorbox">Open scan failed: ${esc(e.message)}</div>`}}async function deleteScan(id){if(confirm('Delete scan?')){await api('/api/scan/'+id,{method:'DELETE'});loadHistory()}}
async function runAI(id){$('result').innerHTML+='<p>Running AI...</p>';let d=await api('/api/scan/'+id+'/ai',{method:'POST',body:'{}'});openScan(id)}
async function loadPresets(){try{let d=await api('/api/presets');let presets=Array.isArray(d.presets)?d.presets:[];let h='<table><tr><th>Name</th><th>Type</th><th>Base</th><th>Mode</th><th>Description</th><th>Action</th></tr>';presets.forEach(p=>h+=`<tr><td>${esc(p.name)}</td><td>${esc(p.type)}</td><td>${esc(p.base)}</td><td>${esc(p.strategy)}</td><td>${esc(p.description||'')}</td><td>${p.type==='custom'?`<button class="btn danger" onclick="deletePreset('${esc(p.name)}')">Delete</button>`:''}</td></tr>`);$('presetBox').innerHTML=h+'</table>';let sel=$('preset');let current=sel.value;sel.innerHTML=presets.map(p=>`<option>${esc(p.name)}</option>`).join('');sel.value=current||'standard'}catch(e){$('presetBox').innerHTML=`<div class="errorbox">Preset load failed: ${esc(e.message)}</div>`}}
...
def insert_scan(params: dict[str, Any]) -> str:
    ...
    conn().execute("""
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,?)
    """, (scan_id, params["target"], params.get("mode", "fast"), params.get("modules", "safe"), params.get("preset", "standard"), params.get("ports"), min(600, max(10, int(params.get("timeout") or 90))), int(bool(params.get("aggressive"))), int(bool(params.get("no_whois"))), int(bool(params.get("ai"))), int(bool(params.get("show_commands"))), params.get("raw_dir"), "queued", now, "Queued and waiting to start"))
    conn().commit()
    return scan_id
...
def run_scan_worker(scan_id: str) -> None:
    ...
    try:
        ...
        timeout = min(600, max(10, int(scan.get("timeout") or 90)))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def send_head_only(self, content_type: str = "text/html; charset=utf-8", status: int = 200) -> None:
        self.send_response(status)
        self.send_header("Content-Type", content_type)
        self.send_header("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0")
        self.send_header("Pragma", "no-cache")
    ...
    def do_HEAD(self) -> None:
        ...
        if path == "/":
            return self.send_head_only("text/html; charset=utf-8")
        if path.startswith("/api/"):
            return self.send_head_only("application/json; charset=utf-8")
        return self.send_head_only("text/plain; charset=utf-8", 404)
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/api/status":
                return self.send_data({"app": APP, "version": VERSION, "author": AUTHOR, "website": WEBSITE, "web_panel": "live-progress-ai-preset-v3", "active_scan": active_scan()})
            if path == "/api/scans":
